...
#pragma omp master
  ...
  void GetSplitInfo(uint32_t min_bin, uint32_t max_bin, uint32_t default_bin, MissingType missing_type, bool default_left, uint32_t threshold,
                    const data_size_t* data_indices, data_size_t num_data, const score_t* ordered_gradients, const score_t* ordered_hessians, SplitInfo *output) const override {
    // not need to split
    ...
    data_size_t gt_count = 0;
    double sum_gt_gradients;
    double sum_gt_hessians;
    data_size_t* default_count = &gt_count;
    ...
    if (missing_type == MissingType::NaN) {
      ...
      if (default_left) {
        missing_default_count = &lte_count;
        sum_missing_default_gradients = &sum_gt_gradients;
        sum_missing_default_hessians = &sum_gt_hessians;
      }
      for (data_size_t i = 0; i < num_data; ++i) {
        const data_size_t idx = data_indices[i];
        const VAL_T bin = iterator.InnerRawGet(idx);
        if (bin < minb || bin > maxb || t_default_bin == bin) {
          ...
          *sum_missing_default_hessians += ordered_hessians[i];
        } else if (bin > th) {
          gt_count++;
  ...
  void GetCategoricalSplitInfo(uint32_t min_bin, uint32_t max_bin, uint32_t default_bin, const uint32_t* threshold, int num_threahold,
                               const data_size_t* data_indices, data_size_t num_data, const score_t* ordered_gradients, const score_t* ordered_hessians, SplitInfo* output) const override {
    if (num_data <= 0) { return; }
    data_size_t lte_count = 0;
    double sum_lte_gradients = 0;
