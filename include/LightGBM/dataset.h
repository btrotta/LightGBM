...
class Dataset {
  ...
  */
  ...
  inline void GetSplitInfo(int feature, const uint32_t* threshold, int num_threshold,  bool default_left,
                                  const data_size_t* data_indices, data_size_t num_data, const score_t* gradients, const score_t* hessians,
                                  SplitInfo* output) const {
    const int group = feature2group_[feature];
    const int sub_feature = feature2subfeature_[feature];
    feature_groups_[group]->GetSplitInfo(sub_feature, threshold, num_threshold, default_left, data_indices, num_data, gradients, hessians, output);
  }
